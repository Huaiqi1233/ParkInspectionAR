...
园区巡检人员使用手机在**现场通过 AR 标记上报问题工单**（覆盖设备破损、运行异常、安全隐患等巡检场景），管理人员在 **Web 管理端**对工单进行查看与状态流转。
...
- **Unity AR**：真机检测水平平面 → 点击放置立方体标记 → 工单表单（标题必填/描述可选/优先级 high·medium·low 三按钮/位置自动）→ 提交成功/失败提示 → 点击已放置标记查看工单详情 → 顶部状态栏显示 AR 状态与平面数 → **自动读取手机 GPS（含精度）+ 截取现场照片一并上报（方案 A+C：跨设备定位）**
...
对照任务书 §7 加分项参考（不要求全部实现，如实标注）：

| 加分项 | 状态 | 说明 |
|---|---|---|
| Unity 应用构建及编译 | ✅ 已完成 | Android APK 构建成功 |
| 跨设备、跨会话定位的合理方案 | ✅ 已完成 | GPS 经纬度（±精度）+ 现场照片 + 高德地图链接（方案 A+C） |
| React 筛选 | ✅ 已完成 | 状态 / 优先级筛选 + 分页 |
| 自动化测试 | ✅ 已完成 | `scripts/acceptance.ps1`（Go 验收）+ `scripts/blackbox-api.ps1`（黑盒 27/27） |
| React 详情 / 编辑 | ⚠️ 部分 | 状态流转 + 删除已实现；未做独立详情页与标题/描述编辑 UI（接口层 PATCH 已支持） |
| 更完整、易用的 UI 和交互 | ⚠️ 部分 | 紧凑底部表单、顶部状态栏、平面可视化；整体仍为轻量原型风格 |
| 登录、权限或人员管理 | ❌ 未实现 | 未选做 |
| 按优先级或状态改变 AR 标记样式 | ❌ 未实现 | 标记统一为青色立方体 |
| 实时数据或 WebSocket | ❌ 未实现 | React 为手动刷新/轮询式取数 |
| Docker / 更规范的工程组织 | ❌ 未实现 | 未容器化 |
| 业务型 AI 功能 | ❌ 未实现 | 未引入大模型（任务书亦不要求） |
